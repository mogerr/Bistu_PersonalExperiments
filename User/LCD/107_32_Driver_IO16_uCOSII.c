...
// #include "delay.h"
// 延时函数改用 LCD_Delay_10ms(uint32_t)
// 适配 uCOSII 改用 OSTimeDlyHMSM
#include "ucos_ii.h"
...
void LCD_WR_DATA(uint16_t LCD_Data)	
{
	...
	GPIO_SetBits(GPIOC,LCD_CS_PIN); //拉高片选段CS
	GPIO_SetBits(GPIOD,LCD_WR_PIN); //拉高写入的引脚，写入数据	
} 
